...
fn insert_example() -> ast.DmlStatement {
  ast.Insert(
    ...
    rows: [
      [
        ast.ValueExpr(xast.StringLiteral("01a2B3c4D5e6f70abcde")),
    ...
    returning: Some([
      ast.ReturningExpr(col_ref("reading_hlc"), None),
      ast.ReturningExpr(col_ref("reading_time"), None),
    ]),
    span: dummy_span(),
  )
}

const insert_expected = "INSERT INTO sensor_reading (reading_hlc, reading, units, sensor_id)
VALUES
...
pub fn insert_matches_the_spec_worked_example_test() {
  assert dml_codegen.insert_to_sql(insert_example()) == insert_expected
}
...
pub fn insert_with_a_default_value_and_no_on_conflict_or_returning_test() {
  ...
  assert dml_codegen.insert_to_sql(stmt)
    == "INSERT INTO s (a, b)\nVALUES\n  (1, DEFAULT);"
}
...
pub fn generate_end_to_end_against_the_given_catalog_test() {
  let assert Ok(#(sql, catalog_after)) =
    dml_codegen.generate(catalog_with_sensor_reading(), insert_source)
  assert sql == insert_expected <> "\n"
...
pub fn generate_standalone_validates_against_an_empty_catalog_test() {
  // `generate_standalone` is `generate(catalog.empty(), source)` — since
  // every `INSERT` needs its target stream to already exist, this can
  // only ever succeed for a source that's self-contained in a way this
  // grammar doesn't support (DML alone can't also declare a stream), so
  // the meaningful thing to prove is that it really does start from an
  // *empty* catalog: `sensor_reading` isn't found, even though it's a
  // perfectly ordinary stream name.
  let assert Error(dml_codegen.SemanticFailure(
    statement_index: 0,
    errors: [dml_semantics.UnknownStream(name: "sensor_reading", span: _)],
  )) = dml_codegen.generate_standalone(insert_source)
}
...
pub fn a_semicolon_inside_a_string_literal_is_not_a_statement_boundary_test() {
  ...
  let assert Ok(#(sql, _catalog)) =
    dml_codegen.generate(catalog_with_a_stream_named_s(), source)
  assert sql
    == "INSERT INTO s (a)\nVALUES\n  ('x;y');\n\n"
    <> "INSERT INTO s (a)\nVALUES\n  ('z');\n"
}
...
fn catalog_with_a_stream_named_s() -> catalog.Catalog {
  let assert Ok(tokens) = lexer.tokenize("CREATE STREAM s (a HLC);")
  let assert Ok(stmt) = ddl_parser.parse(token_stream.new(tokens))
  let assert Ok(cat) = ddl_semantics.analyze(catalog.empty(), stmt)
  cat
}
