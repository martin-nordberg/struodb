...
// free of `gleam/erlang`/`gleam/otp`: `clock_keeper.gleam` is the actor
// wrapper around this module, holding one `ClockState` as its own actor
// state and calling straight through to `new`/`next`/`next_parts`/
// `merge` below on every message — this module has no notion of an
// actor, a `Subject`, or message-passing at all. Splitting it out this
// way is what lets `HlcParts` (and everything else here) build for a
// target `gleam/otp`/`gleam/erlang` don't support (e.g. JavaScript)
// without dragging the actor machinery along.
//-----------------------------------------------------------------------------
