...
import gleam/result
import hlc/clock.{type ClockState, type HlcError, type HlcParts}

//-----------------------------------------------------------------------------
// The actor wrapper around `hlc/clock`'s pure `(time, counter,
// node_id)` state machine: owns one `ClockState` as its own actor state
// and, on every message, calls straight through to `clock.next`/
...
// encoding — lives in `clock.gleam`; this module's own job is
// purely "run that state machine as a process, one call at a time." See
// `clock.gleam`'s own header comment for why the split exists.
//-----------------------------------------------------------------------------
...
pub fn start(
  ...
  use initial_state <- result.try(clock.new(node_id, now))
...
pub fn next(keeper: Subject(ClockMessage)) -> String {
  actor.call(keeper, waiting: call_timeout_ms, sending: Next)
}
...
/// `HlcParts`.
pub fn next_parts(keeper: Subject(ClockMessage)) -> HlcParts {
  actor.call(keeper, waiting: call_timeout_ms, sending: NextParts)
}
...
pub fn merge(
  keeper: Subject(ClockMessage),
  remote: String,
) -> Result(String, HlcError) {
  actor.call(keeper, waiting: call_timeout_ms, sending: fn(reply_to) {
...
pub fn stop(keeper: Subject(ClockMessage)) -> Nil {
  case process.subject_owner(keeper) {
    Ok(pid) -> {
      let monitor = process.monitor(pid)
      actor.send(keeper, StopClock)
      let assert Ok(_down) =
...
fn handle_message(
  ...
  case msg {
    Next(reply_to) -> {
      let #(new_state, value) = clock.next(state)
      actor.send(reply_to, value)
      actor.continue(new_state)
    }

    NextParts(reply_to) -> {
      let #(new_state, parts) = clock.next_parts(state)
      actor.send(reply_to, parts)
      actor.continue(new_state)
    }

    Merge(remote, reply_to) -> {
      case clock.merge(state, remote) {
        Ok(#(new_state, value)) -> {
