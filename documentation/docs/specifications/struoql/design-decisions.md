...
- **Scope, for now: expressions/function calls, `CREATE STREAM`,
  `ALTER STREAM`, and `INSERT`.** See §3.1.
- **Fixed per-stream schema.** See §3.1.
- **`HLC` is not a data type; every stream automatically gets 4 fixed,
  system-populated `_struo_hlc...` columns, never written in
  `CREATE STREAM`** — see §3.3.2. (This supersedes an intervening design
  ...
  reserved and rejected at declaration time — see §2.2.
...
- **`data_type` has a formal grammar**, matching PostgreSQL's own
  ...
  `DOUBLE PRECISION` as the one two-keyword type. See §3.3.1.
- **`column_name` and `constraint_name` must each be unique within a
  ...
  §3.3.1, §3.3.5.
- **Built-in functions are ordinary identifiers, not keywords.** See §3.2.3.
- **`CHECK` constraints must be named — no unnamed form.** A deliberate
  ...
  look up a system-generated name. See §3.3.5.
- **No `UNIQUE`, no foreign keys.** See §3.3.5.
- **All keywords reserved**, and, additionally, an unquoted identifier
  ...
  unquoted in transpiled PostgreSQL SQL. See §2.3, §2.3.5.
- **Expression grammar and precedence follow PostgreSQL**, restricted to
  the operators/keywords StruoDB currently defines. See §3.2.
...
- **All of PostgreSQL's remaining common operators added**: `::`
  ...
  directly. See §2.5.4–§2.5.7, §3.2.1, §3.2.2.
...
- **`ADD COLUMN` requires `OPTIONAL` or a `DEFAULT`/`GENERATED` clause**,
  ...
  value for a new column. See §3.4.2.
...
- **`GENERATED` and the 5 automatic system columns are excluded from the
  ...
  left to its own table-level `DEFAULT clock_timestamp()`. See §4.1.4.
...
- **`INSERT INTO stream_name` doesn't repeat `STREAM`** — matching plain
  SQL rather than `CREATE STREAM`/`ALTER STREAM`'s own pattern. See §4.1.1.
...
- Non-decimal integer literals (`0x`/`0o`/`0b`) — omitted for now (§2.4.2).
- `E'...'` escape strings and `$$...$$` dollar-quoted strings — omitted
  for now (§2.4.3); dollar-quoting in particular may be worth revisiting if
  StruoDB ever grows function-body-like syntax.
- The meaning, if any, of `*` outside arithmetic.
- Whether `column_clause`s (§3.3.1) may repeat, combine freely, or must
  follow a fixed order.
- Whether/how `column_ref` (§3.2.1) will need stream-qualification once
  multi-stream querying exists.
- Any querying/subscribing syntax (§5) — entirely unspecified.
- How "stronger" is verified when replacing a `CHECK` constraint (§3.4.5):
...
- Whether multiple `alter_action`s may really appear in one `ALTER STREAM`
  statement (§3.4.1) — assumed from PostgreSQL, not confirmed.
- Narrowing type changes and cross-family type changes in `ALTER COLUMN
  ... TYPE` (§3.4.4) — e.g. `INT` to `DECIMAL` — presumed disallowed but
  not explicitly specified.
- `LIKE`/`ILIKE`'s `ESCAPE` clause (§3.2.1) — PostgreSQL allows
  `LIKE pattern ESCAPE escape_char`; not included yet.
- `CAST(expr AS type)` as an alternate spelling of `::` (§2.5.7) — the SQL
  standard form, not included since `::` alone was requested.
- `[ ]` array subscript and `.` used within an expression (as opposed to
  as a qualified-name separator, §2.5.8) — the two PostgreSQL precedence-
  table entries §3.2.2 still doesn't populate; no array types exist yet.
- No built-in functions are defined yet (§3.2.3, §3.3.6) — the earlier
  `TIMESTAMPTZ_FROM_HLC` was never implemented and is retired along with
  the `HLC` data type it depended on (§3.3.2); function-call position
