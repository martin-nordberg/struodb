...
streams are shaped (§3.3): an always-explicit column list, `GENERATED`
...
`stream_name` and `column_name` are identifiers (§2.2); `expr` is as defined
in §3.2. Unlike `CREATE STREAM`/`ALTER STREAM` (§3.3, §3.4), the statement
...
`ALTER STREAM` (§3.4) changing a stream's column order or count over time.
...
The 5 automatic system columns (§3.3.2) are excluded from the column list
...
rather than client-supplied — see §3.3.2). No conflict target (a column
...
of conflict, its `_struo_hlc` primary key (§3.3.5 — there's no `UNIQUE`
