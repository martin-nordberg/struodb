...
// free of `gleam/erlang`/`gleam/otp` and of any notion of an actor,
// `Subject`, or message-passing at all: every function here is a plain
// `state -> new_state` (or `state -> Result(new_state, error)`)
// transformation. This used to be paired with `clock_keeper.gleam`, a
// thin actor wrapper holding one `ClockState` as its own actor state and
// calling straight through to `new`/`next`/`next_parts`/`merge` on every
// message — deleted once StruoDB moved to the JavaScript/Bun target,
// since the equivalent "one clock, called synchronously by whoever wants
// the next value" role is now played by `HlcClock`, a plain TypeScript
// class (`service/src/hlc-clock.ts`) wrapping this same state machine.
// Being erlang/otp-free from the start is exactly what made that move
// possible without touching this module at all.
//-----------------------------------------------------------------------------
...
/// contract the actor's own state used to enforce back when it was a
/// private type local to `clock_keeper.gleam` alone, before the split.
