...
pub fn round_trips_the_maximum_supported_width_test() {
  // 8 is the widest width the capacity lookup table covers — the largest
  // value representable in 8 base-62 characters (62^8 - 1) is still a
  // safe JavaScript integer; 9 characters would not be. See
  // `base62.gleam`'s `capacity_for_width` for why.
  let assert Ok(encoded) = base62.encode(12_345, 8)
  assert string.length(encoded) == 8
...
pub fn encode_rejects_a_width_above_the_maximum_supported_test() {
  let assert Error(InvalidWidth(width: 9)) = base62.encode(0, 9)
}
